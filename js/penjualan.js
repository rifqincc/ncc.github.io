import { getSupabase, cachedResepSummaryData, penjualanInputData } from './config.js';
import { formatRp, getNilaiAsli, formatRupiahInput } from './helpers.js';
import { showLoading, hideLoading, showToast } from './ui.js';
import { hasRole } from './auth.js';

// ===== INIT DROPDOWNS =====
export function initBulanTahunDropdowns() {
...
export async function loadMenuDropdownPenjualan() {
    const supabase = getSupabase();
    const { data, error } = await supabase.from('resep').select('id, nama, harga_jual').order('nama');
    if (error) return;
...
export function renderTablePenjualanInput() {
                ...
                <tr class="border-b border-gray-100 dark:border-gray-700 hover:bg-blue-50/30 dark:hover:bg-blue-900/20 transition-colors">
                    ...
                    <td class="p-3 text-center">
                        <input type="number" min="0" step="1" value="${data.qty}" 
                            ...
                            oninput="window.updatePenjualanInput(${menu.id}, 'qty', this.value)" />
                    </td>
                    <td class="p-3 text-right">
                        <div class="flex justify-end items-center gap-1">
                            ...
                            <input type="text" value="${data.harga_jual ? new Intl.NumberFormat('id-ID').format(data.harga_jual) : ''}" 
                                ...
                                oninput="window.formatRupiahInput(this); window.updatePenjualanInput(${menu.id}, 'harga', this.value)" />
                        </div>
                    </td>
                </tr>
            `;
...
export function updatePenjualanInput(id, field, value) {
...
export async function simpanPenjualanMassal() {
    ...
    for (const [id, data] of Object.entries(penjualanInputData)) {
        ...
        if (qty > 0 && harga > 0) {
            dataToInsert.push({ resep_id: parseInt(id), bulan, tahun, qty, harga_jual: harga });
        }
    }
    if (dataToInsert.length === 0) return alert('Tidak ada data penjualan yang valid (Qty > 0 dan Harga > 0).');
    const supabase = getSupabase();
    ...
    if (conflictIds.length > 0) {
        ...
        await supabase.from('penjualan').delete().eq('bulan', bulan).eq('tahun', tahun).in('resep_id', conflictIds);
    }
    showLoading();
    const { error } = await supabase.from('penjualan').insert(dataToInsert);
    hideLoading();
    if (error) {
        showToast('Gagal menyimpan penjualan: ' + error.message, 'error');
    } else {
        showToast(`Berhasil menyimpan ${dataToInsert.length} data penjualan!`, 'success');
...
export function downloadTemplatePenjualan() {
...
export function initiateImportPenjualan(event) {
...
export function eksekusiImportPenjualan(mode) {
    ...
    reader.onload = async (e) => {
        try {
            ...
            const supabase = getSupabase();
            const { data: menus } = await supabase.from('resep').select('id, nama, kategori, sub_kategori');
            const menuMap = {};
            menus.forEach(m => { menuMap[m.nama.toLowerCase().trim()] = m.id; });
            const dataToInsert = [];
            let skipped = 0;
            const bulan = parseInt(document.getElementById('jual-bulan').value);
            const tahun = parseInt(document.getElementById('jual-tahun').value);
            if (!bulan || !tahun) { alert('Pilih bulan dan tahun terlebih dahulu!'); batalImport(); return; }
            rows.forEach(r => {
                ...
                dataToInsert.push({ resep_id: menuId, bulan, tahun, qty, harga_jual: harga });
            });
            ...
            if (conflictIds.length > 0) {
                if (!confirm(`Data penjualan untuk bulan ${bulan}/${tahun} sudah ada untuk beberapa menu.\n\nSimpan akan menimpa data yang sudah ada. Lanjutkan?`)) {
                    hideLoading(); batalImport(); return;
                }
                await supabase.from('penjualan').delete().eq('bulan', bulan).eq('tahun', tahun).in('resep_id', conflictIds);
            }
            const { error } = await supabase.from('penjualan').insert(dataToInsert);
            hideLoading(); batalImport();
            if (error) {
                showToast('Gagal import penjualan: ' + error.message, 'error');
            } else {
                showToast(`Berhasil import ${dataToInsert.length} data penjualan. ${skipped} baris dilewati.`, 'success');
                loadDataPenjualan();
                renderTablePenjualanInput();
            }
        } catch (err) {
            hideLoading();
            showToast('Gagal mengolah file Excel: ' + err.message, 'error');
            batalImport();
        }
    };
...
export function exportPenjualanToExcel() {
    ...
    let query = supabase
        .from('penjualan')
        .select(`id, bulan, tahun, qty, harga_jual, resep:resep_id (id, nama, kategori, sub_kategori)`);
    ...
    query.then(({ data, error }) => {
        ...
        if (!data || data.length === 0) { showToast('Tidak ada data untuk diexport.', 'info'); return; }
...
export async function loadDataPenjualan() {
    const bulan = document.getElementById('filter-data-bulan').value;
    const tahun = document.getElementById('filter-data-tahun').value;
    const menuId = document.getElementById('filter-data-menu').value;
    const supabase = getSupabase();
    let query = supabase
        .from('penjualan')
        .select(`id, bulan, tahun, qty, harga_jual, resep:resep_id (id, nama, kategori, sub_kategori, harga_jual as harga_jual_resep)`);
    if (bulan && bulan !== 'all') query = query.eq('bulan', parseInt(bulan));
    if (tahun && tahun !== 'all') query = query.eq('tahun', parseInt(tahun));
    if (menuId && menuId !== 'all') query = query.eq('resep_id', parseInt(menuId));
    ...
    data.forEach(row => {
        ...
        `;
        if (canDelete) {
            html += `<button onclick="window.hapusPenjualan(${row.id})" class="text-red-500 dark:text-red-400 hover:text-red-700 font-bold text-lg">✕</button>`;
        } else {
    ...
    const btnMassal = document.querySelector('[onclick="window.hapusPenjualanTerpilih()"]');
    if (btnMassal) {
...
export async function hapusPenjualanTerpilih() {
    if (!hasRole('senior_bar')) return alert('Akses ditolak.');
    ...
    showLoading();
    const supabase = getSupabase();
    const { error } = await supabase.from('penjualan').delete().in('id', ids);
    hideLoading();
    if (error) showToast('Gagal hapus massal.', 'error');
    else loadDataPenjualan();
}

export function toggleSelectAllPenjualan() {
